...
6. Ask zero to three high-value decision clusters per round, with no minimum. When deliverable type is materially ambiguous and shapes the remaining questions, resolve it in the first round. Each numbered question must resolve one natural decision cluster; do not bundle independent decisions or ask optional questions merely to make the specification more complete.
7. After every user response, replace superseded conclusions, update the one-sentence task definition, and run the mandatory stop check in the interview engine. Before declaring that no unknown-critical decision remains, check for unresolved alternatives in deliverable type, execution depth, and acceptance. Do not silently choose between materially different deliverables to justify stopping. Stop as soon as the check passes; the answer-directly and no-clarification controls still take precedence over further interviewing.
...
10. Before sending the actual deliverable or handoff, apply the output contract's task-specific quality check. Correct unsupported factual additions and mismatches with confirmed requirements internally; do not create an extra interview or approval step to fix your own output.
